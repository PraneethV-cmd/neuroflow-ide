...
@misc_bp.route("/api/describe", methods=["POST"])
def api_describe():
    """
    Generate statistical summary of the dataset (Describe Node)
    Calculates count, mean, std, min, percentiles, max for numeric columns
    Calculates count, unique, top, freq for non-numeric columns
    """
    try:
        request_data = request.json
        rows = request_data.get("data", [])
        headers = request_data.get("headers", [])
        selected_columns = request_data.get("selected_columns", []) # List of column names to analyze

        if not rows:
            return jsonify({"error": "No data provided"}), 400

        # Convert to numpy array of objects to handle mixed types
        ...
        # If headers not provided, generate default headers
        if not headers:
        ...
        for i in range(n_cols):
            ...
            # Skip if column selection is active and this column is not selected
            ...
            # Robust numeric check: if >70% of non-empty values are numeric, treat as numeric
            ...
            if not non_empty:
                ...
                # If we have enough data points (at least 1)
                if len(non_empty) > 0:
            ...
            if is_numeric:
                ...
                if len(vals) > 0:
                    ...
                    # Kurtosis = E[((x - mu)/sigma)^4] - 3 (Fisher properties, normal = 0)
                    ...
                    # Calculate Unique, Top, Freq for numeric columns too
...
@misc_bp.route("/api/convert_dtypes", methods=["POST"])
def api_convert_dtypes():
    """
    Convert column data types (DataTypeConverter Node)
    Supports: integer, float, string, boolean, datetime
    """
    try:
        request_data = request.json
        rows = request_data.get("data", [])
        headers = request_data.get("headers", [])
        conversions = request_data.get("conversions", {}) # Dict: { "col_name": "target_type" }
        date_formats = request_data.get("date_formats", {}) # Dict: { "col_name": "format_string" }
        ...
        # Let's organize by column index to convert
        ops_by_index = {}
            ...
            for idx, op in ops_by_index.items():
                ...
                try:
                    ...
                    if val is None or (isinstance(val, str) and val.strip() == ''):
                        converted_val = None # Handle empty
                    
                    elif target_type == 'integer':
                        # Handle float strings "12.0" -> 12
                        try:
                            f_val = float(val)
                        ...
                        if s_val in ['true', '1', 'yes', 'y', 'on']:
                            ...
                            converted_val = False
                             ...
                             # Strict boolean: if not explicitly true/false-like, maybe None?
                             converted_val = bool(val) if isinstance(val, (bool, int, float)) else None

                    elif target_type == 'datetime':
                        # Use format if provided, else try auto
                        s_val = str(val).strip()
                        if fmt and fmt != 'auto':
                            # Map simple format strings to python strptime
                            # YYYY-MM-DD -> %Y-%m-%d
                            py_fmt = fmt.replace('YYYY', '%Y').replace('MM', '%m').replace('DD', '%d')
                            dt = datetime.strptime(s_val, py_fmt)
                        else:
                            # Simple auto-detection (very basic)
                            # Try common formats
                    ...
                    new_row[idx] = None # Or keep original? Usually better to nullify invalid casts
...
def health():
    return jsonify({"status": "ok", "service": "ml-api", "implementation": "MODULAR"})
