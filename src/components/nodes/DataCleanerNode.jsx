    ...
    for (const e of incoming) {
      ...
      if (src?.type === 'csvReader') {
        return {
          ...
          rows: src.data?.rows || [],  // Include rows (will be filtered if user loaded filtered data)
          file: src.data?.file
        };
      }
  ...
  const onClean = async () => {
    ...
    try {
      ...
      if (upstreamData.type === 'csv') {
        // Use pre-loaded rows if available (includes filtered data), otherwise parse file
      ...
      setNodes((nds) => nds.map((n) => {
        ...
        return {
          ...
          data: {
            ...
            removedRows: removed // Store removed rows for context menu
          }
        };
      }));
