...
import { trainHierarchicalClustering } from '../../utils/apiClient';
...
const HierarchicalClusteringNode = ({ id, data, isConnectable }) => {
    ...
    const onRun = async () => {
        ...
        try {
            ...
            const result = await trainHierarchicalClustering(X, options);

            if (result.success) {
                const clusterLabels = result.cluster_labels;
                const clusterRepresentatives = result.cluster_representatives;
                ...
                setNodes((nds) => nds.map((n) => {
                    ...
                    return {
                        ...
                        data: {
                            ...
                            cutCriterion: result.cut_criterion,
                            cutValue: result.cut_value,
                            cutHeight: result.cut_height,
                            dendrogram: result.dendrogram,
                            linkageMatrix: result.linkage_matrix,
                ...
                setResults(result);
                setTrainMsg(`Clustering complete! ${result.n_clusters} clusters formed.`);
    ...
    return (
        <div className="linear-regression-node">
            <InfoButton nodeType="hierarchicalClustering" />
            <Handle type="target" position={Position.Top} isConnectable={isConnectable} style={{ background: '#555' }} />

            <div className="node-header">
                <FaLayerGroup className="node-icon" />
                <span className="node-title">{data.label || 'Hierarchical Clustering'}</span>
            </div>
...
export default HierarchicalClusteringNode;
